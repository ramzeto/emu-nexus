...
MessageDialog::MessageDialog(GtkWindow *parent, string message, string positiveButtonText, string negativeButtonText) : Dialog(parent, "MessageDialog.ui", "messageDialog")
{
    ...
    g_signal_connect (positiveButton, "clicked", G_CALLBACK (+[](GtkButton *button, gpointer messageDialog) -> void {
        ((MessageDialog *)messageDialog)->positive();
    }), this);
    
    g_signal_connect (negativeButton, "clicked", G_CALLBACK (+[](GtkButton *button, gpointer messageDialog) -> void {
