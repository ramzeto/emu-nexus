...
{        
    ...
    signalSizeAllocateHandlerId = g_signal_connect (getPanelBox(), "size-allocate", G_CALLBACK(signalSizeAllocate), this);    
    signalSizeEdgeReachedHandlerId = g_signal_connect (gameGridScrolledWindow, "edge-reached", G_CALLBACK(signalGameGridScrolledWindowEdgeReached), this);
    ...
    NotificationManager::getInstance()->registerToNotification(NOTIFICATION_GAME_EDIT_REQUIRED, this, callbackNotification, 1);
}
...
void GamesPanel::removeGame(int64_t gameId)
{
    ...
    if(messageDialog->execute() == GTK_RESPONSE_YES)
    {
        ...
        NotificationManager::getInstance()->postNotification(NOTIFICATION_GAME_FAVORITE_UPDATED, new Game(*game));
...
void GamesPanel::signalSizeAllocate(GtkWidget* widget, GtkAllocation* allocation, gpointer gamesPanel)
{    
    if(((GamesPanel *)gamesPanel)->isDestroyed())
    {
        return;
    }
    ...
    {        
        ...
        g_timeout_add(10, callbackFirstShowHackyTimeout, gamesPanel);
    }
}

void GamesPanel::signalGameGridScrolledWindowEdgeReached(GtkScrolledWindow* scrolledWindow, GtkPositionType positionType, gpointer gamesPanel)
{        
...
gint GamesPanel::callbackFirstShowHackyTimeout(gpointer gamesPanel)
{
...
void GamesPanel::callbackNotification(CallbackResult *callbackResult)
{
    try
    {
        GamesPanel *gamesPanel = (GamesPanel *)callbackResult->getRequester();        
        /*if(gamesPanel->isDestroyed())
        {
            return;
        }*/
        ...
        else if(callbackResult->getType().compare(NOTIFICATION_GAME_EDIT_REQUIRED) == 0)
        {
            Game *game = (Game *)callbackResult->getData();
