...
<span size="large">ROM files could be uncompressed on the go. For multidisk games, if multiple .cue files are present in a compressed ROM, a <b>00000_autogenerated.m3u</b> file with all the .cue files referenced in order is generated, so this ROM can be easily launched using emulators like <a href="https://libretro.readthedocs.io/en/latest/library/beetle_saturn/">Retroarch with the Beetle Saturn core</a>. The uncompressed files are stored in cache so they can be used directly next time. Tar, Tar.gz, Zip and 7zip formats are supported.</span>
